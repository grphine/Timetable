    ...
    */
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath){
        if editingStyle == .delete{
            ...
            //there's the problem where the queue is regenerated by pulling data from the whole. yet, this item is not to appear. maybe create a blacklist array? [event, time]. any items in that are not loaded. when user closes app, this data is lost
            
            //i need to rework the pulling function so it does not pull all items indiscriminately. it can't pull items after a certain time.
            //compare current time to hour of occurence of event, to decide whether to generate or not.
            //just keep them local, no one will notice
            
            //find item index
            //find which queue it is in
            //link nodes around it
                //keep node before
                //node.next = node to delete
                //node.next.next = node to link to
